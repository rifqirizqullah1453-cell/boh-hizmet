...
/**
 * Creates all cleaning tables if they don't already exist, then seeds
 * reference data using INSERT IGNORE so re-runs are safe.
 * Called once on server startup — idempotent.
 */
export async function ensureCleaningSchema(db: Database): Promise<void> {
  const DDL: string[] = [
    ...
    `CREATE TABLE IF NOT EXISTS \`cleaning_property_types\` (
      \`slug\` varchar(50) NOT NULL,
      \`label\` varchar(100) NOT NULL,
      \`emoji\` varchar(10),
      \`sortOrder\` int NOT NULL DEFAULT '0',
    ...
    `CREATE TABLE IF NOT EXISTS \`cleaning_base_rates\` (
    ...
    `CREATE TABLE IF NOT EXISTS \`cleaning_room_fees\` (
    ...
    `CREATE TABLE IF NOT EXISTS \`cleaning_dirt_levels\` (
    ...
    // Seed service types
    `INSERT IGNORE INTO \`cleaning_service_types\` (\`slug\`,\`label\`,\`description\`,\`priceMultiplier\`,\`durationMultiplier\`,\`sortOrder\`) VALUES
    ...
    // Seed property types
    `INSERT IGNORE INTO \`cleaning_property_types\` (\`slug\`,\`label\`,\`emoji\`,\`sortOrder\`) VALUES
      ('rumah','Rumah','🏠',0),
      ('apartemen','Apartemen','🏢',1),
      ('kantor','Kantor','🏗️',2),
      ('kos','Kos','🛏️',3)`,
    // Seed base rates
    `INSERT IGNORE INTO \`cleaning_base_rates\` (\`id\`,\`propertyTypeSlug\`,\`areaMin\`,\`areaMax\`,\`basePrice\`,\`baseDurationHours\`,\`baseWorkers\`) VALUES
    ...
    // Seed room fees
    `INSERT IGNORE INTO \`cleaning_room_fees\` (\`roomType\`,\`pricePerUnit\`,\`durationMinPerUnit\`) VALUES
      ('bedroom',50,30),
      ('bathroom',75,45)`,
    // Seed dirt levels
    `INSERT IGNORE INTO \`cleaning_dirt_levels\` (\`slug\`,\`label\`,\`priceMultiplier\`,\`durationMultiplier\`,\`sortOrder\`) VALUES
      ('ringan','Ringan','1.00','1.00',0),
      ('sedang','Sedang','1.20','1.20',1),
      ('berat','Berat','1.50','1.50',2)`,
    // Seed addons
    `INSERT IGNORE INTO \`cleaning_addons\` (\`slug\`,\`label\`,\`emoji\`,\`price\`,\`durationMin\`,\`active\`,\`sortOrder\`) VALUES
    ...
    // Seed config
    `INSERT IGNORE INTO \`cleaning_config\` (\`key\`,\`value\`,\`description\`) VALUES
  ...
  console.log("[cleaning] Schema and seed data verified.");
}
