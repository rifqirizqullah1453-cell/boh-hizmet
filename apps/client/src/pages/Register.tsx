...
export default function Register() {
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      ...
      if (role === 'worker') {
        // Pre-fill name from Google account then let worker fill in details
        const fbName = auth.currentUser?.displayName ?? '';
        if (fbName) setName(fbName);
        setUsedGoogle(true);
        setStep(3);
  ...
  const submit = async () => {
    setLoading(true);
    try {
      if (usedGoogle && role === 'worker') {
        // Already authenticated via Google — just persist role in DB
        const googleName = auth.currentUser?.displayName || name || 'Worker';
        await registerMut.mutateAsync({ name: googleName, role: 'worker' });
        if (phone) await updateProfileMut.mutateAsync({ phone });
      } else {
        if (!name || !email || !password) { toast('All fields required', 'error'); setLoading(false); return; }
