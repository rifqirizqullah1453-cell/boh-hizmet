  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        console.info("[useFcmToken] FCM token saved");

        // Handle foreground messages — show an in-page alert via the browser Notification API.
        onMessage(messaging, (payload) => {
          ...
          if (Notification.permission === "granted") {
            new Notification(title, {
              ...
              tag: payload.data?.orderId ?? "boh-worker-fg",
            });
          }
        });
      } catch (err) {
